		...
		public function column_default( $rec, $column_name ) {
			...
			switch ( $column_name ) {
				...
				case 'variation':
					if ( $product->is_type( 'variation' ) ) {
						...
						foreach ( $variations as $key => $value ) {
							...
							$key  	= urldecode( ucfirst( str_replace( array( 'attribute_pa_', 'attribute_'), '', $key ) ) ); // be compatible with no latin charset
							$value 	= urldecode( $value ); // be compatible with no latin charset
				...
				case 'actions':
					...
					// get search query args to keep view
					$sq_args = array();
					! empty( $_GET['s'] ) && $sq_args['s'] = sanitize_text_field( $_GET['s'] );
					! empty( $_GET['status'] ) && $sq_args['status'] = sanitize_text_field( $_GET['status'] );

					$delete_query_args = array_merge( array(
						'page'   => $_GET['page'],
						'tab'    => $_GET['tab'],
						'action' => 'delete',
						'id'     => $product_id,
					...
					$mail_query_args = array_merge( array(
						'page'   => $_GET['page'],
						'tab'    => $_GET['tab'],
						'action' => 'send_mail',
						'id'     => $product_id,
