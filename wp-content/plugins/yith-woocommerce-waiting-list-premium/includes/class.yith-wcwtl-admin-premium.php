	...
	class YITH_WCWTL_Admin_Premium extends YITH_WCWTL_Admin {
		...
		public function exclusions_table() {
			if ( file_exists( YITH_WCWTL_DIR . '/templates/admin/exclusions-tab.php' ) ) {
				...
				// remove WC script to prevent incompatibility issue
		...
		public function table_actions() {
			...
			! empty( $_GET['s'] ) && $list_query_args['s'] = sanitize_text_field( $_GET['s'] );
			! empty( $_GET['status'] ) && $list_query_args['status'] = sanitize_text_field( $_GET['status'] );
			...
			elseif ( 'send_mail' == $action && isset( $_GET['id'] ) ) {
				...
				if ( get_option( 'yith-wcwtl-enable' ) !== 'yes' ) {
					...
					$users   = isset( $_GET['user'] ) ? explode( ',', $_GET['user'] ) : array();

					do_action( 'yith_wcwtl_schedule_email_send', $product, $users );
					$mess     = 5;
				}
