...
class Utils {
	...
	public static function get_bought_ids( $line_items ) {
		...
		foreach ( $line_items as $item ) {
			...
			$p_id      = isset( $item_data['variation_id'] ) && ! empty( $item_data['variation_id'] ) ? $item_data['variation_id'] : $item_data['product_id'];
			$bought[]  = $p_id;
		}
	...
	public static function get_categories_from_bought_id( $bought_ids ) {
		...
		foreach ( $bought_ids as $id ) {
			...
			if ( ! empty( $parent_id ) ) {
				$cats = wc_get_product_cat_ids( $parent_id );
			} else {
	...
	public static function get_hide_rules_data() {
		$r = [
			'default'                 => [ 'min_order', 'max_order', 'category', 'products', 'payment_methods', 'price_type_order', 'country' ],
			'customer_new_account'    => [ 'min_order', 'max_order', 'category', 'products', 'payment_methods', 'price_type_order' ],
			'customer_reset_password' => [ 'min_order', 'max_order', 'category', 'products', 'payment_methods', 'price_type_order' ]
		];
	...
	public static function default_shortcode_for_replace() {
		...
		return [
			...
			'{gzd_customer_activation_url}' => $myaccount_url ?? '',
			...
			if ( $object ) {
				...
				if ( property_exists( $object, 'object' ) && is_a( $object->object, 'WP_User' ) ) {
					...
					if ( isset( $object->object->ID ) ) {
						...
						/*Germanized for WooCommerce activation url*/
						if ( class_exists( 'WC_GZD_Customer_Helper' ) ) {
							if ( \WC_GZD_Customer_Helper::instance()->is_double_opt_in_enabled() ) {
								$WC_GZD_Customer_Helper                      = \WC_GZD_Customer_Helper::instance();
								$user_activation                             = $WC_GZD_Customer_Helper->get_customer_activation_meta( $user_id );
								$user_activation_url                         = $WC_GZD_Customer_Helper->get_customer_activation_url( $user_activation );
								$shortcodes['{gzd_customer_activation_url}'] = $user_activation_url;
