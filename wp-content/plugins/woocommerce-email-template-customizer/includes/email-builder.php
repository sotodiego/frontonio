...
//	    Send test result
		...
		add_action( 'admin_notices', array( $this, 'viwec_notice_filter' ) );
	}

	/**
	 * Viwec filter email template notice
	 */
	public function viwec_notice_filter() {
		global $pagenow;
		...
		if ( is_admin() && $pagenow == 'edit.php' && $post_type == 'viwec_template' && ! empty( $_GET['viwec_template_filter'] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            <div class="villatheme-dashboard notice notice-warning">
                <div class="villatheme-content">
                    <p><?php esc_html_e( 'Priority Rule Note: The plugin checks rules from top to bottom of Email template list to find a matching template. When filter email by type, templates with higher priority rules will be arranged to the higher position and are applied first. Go to each email template editing page to add priority for each one.', 'viwec-email-template-customizer' ); ?></p>
                </div>
            </div>
			<?php
		}
	...
	public function save_post( $post_id ) {
		...
		foreach ( $keys as $key ) {
			...
			if ( isset( $_POST[ $key ] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
				$value = $key == 'viwec_email_structure' ? wp_filter_post_kses( htmlentities( $_POST[ $key ] ) ) : wc_clean( $_POST[ $key ] );// phpcs:ignore WordPress.Security.NonceVerification.Missing
			}
	...
	public function preview_template() {
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'viwec_nonce' ) ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}

		$data = isset( $_POST['data'] ) ? json_decode( wp_unslash( html_entity_decode( wp_filter_post_kses( htmlentities( $_POST['data'] ) ) ) ), true ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		printf( '<style type="text/css">%s</style>', ( $custom_css ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		wp_die();
	}

	public function preview_custom_css() {
		$custom_css = isset( $_POST['custom_css'] ) ? sanitize_text_field( $_POST['custom_css'] ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function send_test_email() {
		...
		$subject    = apply_filters( 'viwec_sent_test_subject', esc_html__( 'WooCommerce Email Customizer test email template', 'viwec-email-template-customizer' ), $post_ID );
	...
	public function background_image_style() {
		if ( get_current_screen()->id == 'viwec_template' ) {
			...
			wp_register_style( 'viwec-inline-style', false, VIWEC_VER );
                ...
                jQuery(document).ready(function ($) {
                    $('.viwec-email-status').on('change', function () {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            dataType: 'json',
                            data: {action: 'viwec_set_email_status', status, id, nonce: '<?php echo esc_attr( wp_create_nonce( 'viwec_set_email_status' ) ) ?>'},
                            success: function () {
	...
	public function duplicate_template() {
		...
		$dup_id = ! empty( $_GET['id'] ) ? sanitize_text_field( $_GET['id'] ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( $dup_id ) {
	...
	public function parse_query_filter( $query ) {
		...
		if ( is_admin() && $pagenow == 'edit.php' && $post_type == 'viwec_template' && ! empty( $_GET['viwec_template_filter'] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$query->query_vars['meta_value']   = sanitize_text_field( $_GET['viwec_template_filter'] );// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function add_edit_buttons( $email ) {
		...
		printf( "<a href='%s' class='viwec-edit-button' title='%s'><i class='dashicons dashicons-edit'> </i></a>", esc_url( $href ), esc_html( $title ) );
		$email_statuses = [
