...
function viwec_render_email_template( $id ) {
	$email_template = VIWEC\INCLUDES\Email_Render::init( [ 'template_id' => $id ] );
...
if ( ! function_exists( 'viwec_fix_get_formatted_meta_data' ) ) {
	function viwec_fix_get_formatted_meta_data( $formatted_meta, \WC_Order_Item $object ) {
		remove_filter( 'woocommerce_order_item_get_formatted_meta_data', 'viwec_fix_get_formatted_meta_data' );

		return $object->get_formatted_meta_data();
	}
}