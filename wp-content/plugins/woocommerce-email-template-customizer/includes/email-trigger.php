...
//		Email with wc_mail
		add_action( 'woocommerce_email_header', array( $this, 'send_email_via_wc_mailer' ), 0, 2 );
...
//		Minify email content
	...
	public function trigger_recipient( $recipient, $object, $class_email ) {
		...
		if ( ! $object ) {
            $viwec_accept_email_type = apply_filters('viwec_accept_email_type',[]);
            if (!is_array($viwec_accept_email_type) || !in_array($class_email->id, $viwec_accept_email_type)) {
	            return $recipient;
            }
		}
	...
	public function subject_shortcode( $args ) {
		...
		if ( $this->object && $meta_key ) {
			if ( $meta_customer_order === 'yes' ) {
				...
				$meta_value = (array) $this->object->get_meta( $meta_key, true );
			}
		...
		if ( is_array( $meta_value ) ) {
			switch ( $meta_key ) {
				case '_wc_shipment_tracking_items':
					$get_shipment_tracking = (array) $meta_value[0];

					if ( isset( $get_shipment_tracking[ $array_key ] ) ) {
						$meta_value = (array) $get_shipment_tracking[ $array_key ];
					} else {
						$meta_value = wp_json_encode( (array) $get_shipment_tracking );
					}
					break;
				default:
					if ( isset( $meta_value[ $array_key ] ) ) {
						$meta_value = (array) $meta_value[ $array_key ];
					} elseif (count($meta_value) === 1 && is_string($meta_value[0])) {
	...
	public function replace_subject( $subject, $object ) {
		if ( $this->template_id ) {
			...
			$subject  = htmlspecialchars_decode( do_shortcode( $subject ) ,ENT_QUOTES);
			if ($wc_symbol){
...
//			$lang_code = function_exists( 'icl_get_languages' ) ? get_post_meta( $order->get_id(), 'wpml_language', true ) : '';
			$lang_code = function_exists( 'icl_get_languages' ) ? $order->get_meta( 'wpml_language' ) : '';
		} else if ( class_exists( 'TRP_Translate_Press' ) ) {
			$lang_code = $order->get_meta( 'trp_language' );
		}
		...
		foreach ( $posts as $post ) {
			...
			if ( ! empty( $rule_categories ) && ! empty( $rule_products ) ) {
				...
				if (
					! count( array_intersect( $categories, (array) $rule_categories ) ) ||
					! count( array_intersect( $bought_ids, (array) $rule_products ) )
				) {
					continue;
				}
			...
			$rule_3rd = true;
			$rule_3rd = apply_filters( 'viwec_add_rule_template_3rd', $rule_3rd, $rules, $order );
			if ( ! $rule_3rd ) {
				continue;
			}
			/*Rule for price*/
		...
		$log     = wc_get_logger();
		$log->log( 'DEBUG',
			 print_r( $filter_ids, true ) ,
			array(
				'source' => 'VIWEC',
			)
		);

		return apply_filters( 'viwec_find_email_template_id_with_rule_order', current( $filter_ids ), $posts, $country_code, $categories, $order_price , $order);
	}
	...
	public function get_template_id_no_order( $type, $object = [] ) {
		...
		if ( ! empty( $_POST['billing_country'] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$country_code = sanitize_text_field( $_POST['billing_country'] );// phpcs:ignore WordPress.Security.NonceVerification.Missing
		} else {
		...
		if ( $country_code ) {
			foreach ( $posts as $post ) {
				...
				/*Rule compatible with 3rd*/
				$rule_3rd = true;
				$rule_3rd = apply_filters( 'viwec_add_rule_template_3rd', $rule_3rd, $rules, $object );
	...
	public function replace_wp_new_user_email( $wp_new_user_notification_email, $user, $blogname ) {
		...
		if ( $this->template_id ) {
			...
			$register_data = [];
			if ( isset( $_POST['action'] ) && $_POST['action'] == 'uael_register_user' ) {// phpcs:ignore WordPress.Security.NonceVerification.Missing
				$data = wc_clean( $_POST['data'] );// phpcs:ignore WordPress.Security.NonceVerification.Missing
			} else {
	...
	public function send_email_via_wc_mailer( $heading, $email ) {
	    /*If email empty is template is preview*/
