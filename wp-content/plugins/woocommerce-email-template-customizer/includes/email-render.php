...
//		}
	...
	public function shortcode( $args ) {
		$args                = wp_parse_args( $args, [ 'key' => '', 'meta_customer_order' => '', 'array_key' => '' ] );
		$meta_key            = sanitize_text_field( $args['key'] );
		$meta_customer_order = sanitize_text_field( $args['meta_customer_order'] );
		$array_key           = sanitize_text_field( $args['array_key'] );
		$meta_value          = '';
		...
		if ( $this->order && $meta_key ) {
			if ( $meta_customer_order === 'yes' ) {
				...
				$meta_value = $this->order->get_meta( $meta_key, true );
			}
		...
		if ( is_array( $meta_value ) ) {
			switch ( $meta_key ) {
				case '_wc_shipment_tracking_items':
					$get_shipment_tracking = (array) array_shift( array_values( $meta_value ) );

					if ( isset( $get_shipment_tracking[ $array_key ] ) ) {
						$meta_value = $get_shipment_tracking[ $array_key ];
					} else {
						$meta_value = $get_shipment_tracking;
					}
					break;
				default:
					if ( isset( $meta_value[ $array_key ] ) ) {
						$meta_value = $meta_value[ $array_key ];
					}
					break;
			}
	...
	public function demo_order() {
		...
		$order->set_total( 60 );
		$order->set_date_created( current_time( 'U' ) );
		$order->set_date_paid( current_time( 'U' ) );
		$this->order = $order;
	}
	...
	public function render( $data ) {
		...
		if ( $this->preview ) {
			$this->direction = isset( $_POST['direction'] ) ? sanitize_text_field( $_POST['direction'] ) : 'ltr';// phpcs:ignore WordPress.Security.NonceVerification.Missing
		}
		...
		<?php
		$email_body = ob_get_clean();// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		$email_body_minify = Utils::minify_html( $email_body );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo '<div class="viwec-ios-preview-hidden" style="display: none;color: #ffffff;size: 2pt;">' . wp_strip_all_tags( $email_body_minify ) . '</div>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $email_body;// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		$this->email_footer();
		if ( class_exists( 'WooCommerce_Germanized' ) || apply_filters( 'viwec_enable_woocommerce_email_order_details', false ) ) {
			add_action( 'woocommerce_email_order_details', [ \WC_Emails::instance(), 'order_downloads' ], 90 );
			add_action( 'woocommerce_email_order_details', [ \WC_Emails::instance(), 'order_details' ], 90 );
		}
	}
								...
								foreach ( $row['cols'] as $key => $col ) {
									...
									if ( $start == $key ) { ?>
                                        ...
                                        <table align="center" width="100%" role="presentation" border="0" cellpadding="0"
                                            ...
                                            <tr>
                                                <td valign='middle' style="vertical-align:middle;width:<?php echo esc_attr( $width ) ?>;">
                                        <![endif]-->
									<?php } ?>
                                        ...
                                        <tr>
                                            <td>
                                                <table width='100%' align='left' border='0' cellpadding='0' cellspacing='0' style='margin:0; padding:0;border-collapse: collapse;width: 100%'>
                                                    <tr>
                                                        <td class="viwec-column" valign='middle' width='100%' style='line-height: 1.5;box-sizing: border-box;<?php echo esc_attr( $col_style ) ?>'>
															<?php
															if ( ! empty( $col['elements'] && is_array( $col['elements'] ) ) ) {
																foreach ( $col['elements'] as $el ) {
                                                                    ...
                                                                    <table align='center' width='100%' border='0' cellpadding='0' cellspacing="0" style='border-collapse: collapse;'>
                                                                        <tr>
                                                                            <td valign="<?php echo esc_attr($type ==='html_image' ?'middle': 'top'); ?>" style='<?php echo esc_attr( $el_style ); ?>'>
                                                                                <table class="<?php echo esc_attr( $type ) ?>" align='center' width='100%' border='0' cellpadding='0' cellspacing="0" style='border-collapse:
                                                                                                collapse;'>
                                                                                    <tr>
                                                                                        <td valign="<?php echo esc_attr($type ==='html_image' ?'middle': 'top'); ?>" dir="<?php echo esc_attr( $this->direction ) ?>"
...
//				remove_filter( 'woocommerce_order_item_get_formatted_meta_data', [ $this, 'remove_autop' ] );
	...
	public function remove_autop( $formatted_meta ) {
		if ( ! empty( $formatted_meta ) ) {
			...
			foreach ( $formatted_meta as $id => $meta ) {
				$meta->display_value   = make_clickable( wp_strip_all_tags( $meta->display_value ) );
		...
		if ( $this->order ) {
			...
			if ( ! empty( $item_totals ) && is_array( $item_totals ) ) {
				foreach ( $item_totals as $id => $item ) {
					...
					if ( apply_filters( 'viwec_hidden_subtotal_tax', __return_false() ) && $id == 'tax' ) {
						continue;
					}
	...
	public function render_html_order_total( $props, $arr_style ) {
		...
		if ( $this->order ) {
			...
			$tax_display   = apply_filters( 'viwec_total_type_of_tax_display', get_option( 'woocommerce_tax_display_cart' ) );/*Only accepts the values "incl" and "excl". Set other 2 values to hide them*/
	...
	public function render_html_billing_address_via_hook( $order, $sent_to_admin, $plain_text, $email, $props, $arr_style = [] ) {
		...
		if ( apply_filters( 'viwec_show_billing_phone', true )) {
			...
			if ( apply_filters( 'viwec_remove_billing_phone_link', false ) ) {
	...
	public function render_html_shipping_address_via_hook( $order, $sent_to_admin, $plain_text, $email, $props, $arr_style = [] ) {
		...
		if ( ! empty( $order_data['shipping']['phone'] )&& apply_filters( 'viwec_show_shipping_phone', true ) ) {
			...
			if ( apply_filters( 'viwec_remove_shipping_phone_link', false ) ) {
	...
	public function render_html_wc_hook( $props, $arr_style = [] ) {
		...
		if ( $this->preview ) {
			...
			if ( ! $content ) {
				...
				<?php
				$content = ( ob_get_clean() );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			$content = $class_email->style_inline( "<div class='{$hook}'>" . $content . '</div>' );
			echo str_replace( [ 'margin-bottom: 40px;' ], [ '' ], $content );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		} else {
			$args = $this->template_args;
			...
			echo "<div class='{$hook}'>";// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			if ( isset( $args['order'] ) && $args['order'] !== null ) {
	...
	public function render_html_recover_heading( $props, $arr_style = [] ) {
		...
		if ( $this->recover_heading && ! $rendered_heading ) {
			echo( $this->recover_heading );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function render_html_recover_content( $props, $arr_style = [] ) {
		...
		if ( $this->preview ) {
			...
			$class_email = new \WC_Email();
			ob_start();
			printf( "<p>Hi %s, Just to let you know — we've received your order #%s, and it is now being processed:</p>", esc_html( $this->order->get_billing_first_name() ), esc_html( $this->order->get_id() ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			...
			$content = '<div id="viwec-transferred-content">' . ( $content ) . '</div>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		if ( $this->other_message_content ) {
			...
			$content = preg_replace( '/border=[\'"]\d+[\'"]/', 'border="0"', $content );
			echo '<div id="viwec-transferred-content">' . $content . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
			...
			foreach ( $subscriptions as $subscription ) {
                ...
                <tr>
					...
					foreach ( $cells as $cell_content ) {
						printf( "<td style='%s'>%s</td>", esc_attr( $style ), $cell_content );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					?>
                </tr>
				<?php
		...
		foreach ( $subscriptions as $subscription ) {
			...
			if ( is_file( VIWEC_TEMPLATES . "order-items/subscriptions-switched-items.php" ) ) {
				...
				if ( $title ) {
					...
					$style = $this->get_style( $props, 'childStyle', '.viwec-wc-subscriptions-title' );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					$style .= 'font-weight:bold;padding-bottom:6px;';
					printf( '<div style="%s">%s</div>', $style, $title );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				?>
                <table width='100%' border='0' cellpadding='0' cellspacing='0' align='center'>
                    <tr>
	...
	public function get_suggest_products( $suggest_type, $limit, $props, $first_variation = false ) {
		...
		$suggest_products = $return_products = $categories_id = $bought_ids = [];
		$orderby          = 'rand';
		...
		switch ( $suggest_type ) {
			...
			case 'category':
				foreach ( $bought_ids as $id ) {
					...
					$parent_id       = $current_product ? $current_product->get_parent_id() : 0;
					if ( ! empty( $parent_id ) ) {
						$categories_id = wc_get_product_cat_ids( $parent_id );
					} else {
						$categories_id = wc_get_product_cat_ids( $id );
					}
				}
				break;
		...
		$args = [
			...
			'product_category_id' => $categories_id,
			'orderby'             => $orderby,
			'order'               => 'DESC',
		...
		foreach ( $downloads as $download ) {
			if ( $pid == $download['product_id'] ) {
				...
				if ( ! empty( $download['access_expires'] ) ) {
					$datetime     = esc_attr( date( 'Y-m-d', strtotime( $download['access_expires'] ) ) );// phpcs:ignore 	WordPress.DateTime.RestrictedFunctions.date_date
				...
				echo "<p><a href='$href' style='" . $style . "'>" . apply_filters( 'viwec_download_product_link_text', $display ) . "</a> " . $expires . "</p>";// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
