...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_13');

function request_shipping_quote_method_13() {
	if (!class_exists('WC_Portugal_Optica')) {

		class WC_Portugal_Optica extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_13');

function add_request_shipping_quote_13( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Portugal_Optica::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Portugal_Optica';
	}
