...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_9');

function request_shipping_quote_method_9() {
	if (!class_exists('WC_Internacional_Express')) {

		class WC_Internacional_Express extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_9');

function add_request_shipping_quote_9( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Internacional_Express::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Internacional_Express';
	}
