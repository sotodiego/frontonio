...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_8');

function request_shipping_quote_method_8() {
	if (!class_exists('WC_Islas_Maritimo')) {

		class WC_Islas_Maritimo extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_8');

function add_request_shipping_quote_8( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Islas_Maritimo::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Islas_Maritimo';
	}
