...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_3');

function request_shipping_quote_method_3() {
	if (!class_exists('WC_Paq_24')) {

		class WC_Paq_24 extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_3');

function add_request_shipping_quote_3( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_24::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_24';
	}
