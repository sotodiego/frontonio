...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_25');

function request_shipping_quote_method_25() {
	if (!class_exists('WC_Paq_24_Oficina_Elegida')) {

		class WC_Paq_24_Oficina_Elegida extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_25');

function add_request_shipping_quote_25( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_24_Oficina_Elegida::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_24_Oficina_Elegida';
	}
