...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_5');

function request_shipping_quote_method_5() {
	if (!class_exists('WC_EPaq_24')) {

		class WC_EPaq_24 extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_5');

function add_request_shipping_quote_5( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_EPaq_24::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_EPaq_24';
	}
