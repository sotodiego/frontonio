...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_12');

function request_shipping_quote_method_12() {
	if (!class_exists('WC_Campania_Cex')) {

		class WC_Campania_Cex extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_12');

function add_request_shipping_quote_12( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Campania_Cex::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Campania_Cex';
	}
