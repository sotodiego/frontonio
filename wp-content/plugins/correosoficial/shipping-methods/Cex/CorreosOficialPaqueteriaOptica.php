...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_14');

function request_shipping_quote_method_14() {
	if (!class_exists('WC_Paqueteria_Optica')) {

		class WC_Paqueteria_Optica extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_14');

function add_request_shipping_quote_14( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paqueteria_Optica::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paqueteria_Optica';
	}
