...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_4');

function request_shipping_quote_method_4() {
	if (!class_exists('WC_Paq_Empresa_14')) {

		class WC_Paq_Empresa_14 extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_4');

function add_request_shipping_quote_4( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Empresa_14::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Empresa_14';
	}
