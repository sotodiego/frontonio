...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_11');

function request_shipping_quote_method_11() {
	if (!class_exists('WC_Entrega_Plus')) {

		class WC_Entrega_Plus extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_11');

function add_request_shipping_quote_11( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Entrega_Plus::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Entrega_Plus';
	}
