...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_1');

function request_shipping_quote_method_1() {
	if (!class_exists('WC_Paq_10')) {

		class WC_Paq_10 extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_1');

function add_request_shipping_quote_1( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_10::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_10';
	}
