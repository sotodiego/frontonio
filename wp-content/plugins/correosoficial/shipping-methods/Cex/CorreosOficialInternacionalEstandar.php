...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_10');

function request_shipping_quote_method_10() {
	if (!class_exists('WC_Internacional_Estandar')) {

		class WC_Internacional_Estandar extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_10');

function add_request_shipping_quote_10( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Internacional_Estandar::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Internacional_Estandar';
	}
