...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_7');

function request_shipping_quote_method_7() {
	if (!class_exists('WC_Islas_Documentacion')) {

		class WC_Islas_Documentacion extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_7');

function add_request_shipping_quote_7( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Islas_Documentacion::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Islas_Documentacion';
	}
