...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_27');

function request_shipping_quote_method_27() {
	if (!class_exists('WC_Paquete_Postal_Economico_Internacional')) {

		class WC_Paquete_Postal_Economico_Internacional extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_27');

function add_request_shipping_quote_27( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paquete_Postal_Economico_Internacional::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paquete_Postal_Economico_Internacional';
	}
