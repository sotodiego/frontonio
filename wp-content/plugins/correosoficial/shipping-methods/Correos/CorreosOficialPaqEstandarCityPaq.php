...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_20');

function request_shipping_quote_method_20() {
	if (!class_exists('WC_Paq_Estandar_City_Paq')) {

		class WC_Paq_Estandar_City_Paq extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_20');

function add_request_shipping_quote_20( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Estandar_City_Paq::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Estandar_City_Paq';
	}
