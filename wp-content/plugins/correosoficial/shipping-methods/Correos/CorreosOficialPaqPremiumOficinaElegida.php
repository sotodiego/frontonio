...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_16');

function request_shipping_quote_method_16() {
	if (!class_exists('WC_Paq_Premium_Oficina_Elegida')) {

		class WC_Paq_Premium_Oficina_Elegida extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_16');

function add_request_shipping_quote_16( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Premium_Oficina_Elegida::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Premium_Oficina_Elegida';
	}
