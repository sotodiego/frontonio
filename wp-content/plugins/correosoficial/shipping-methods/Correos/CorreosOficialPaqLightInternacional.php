...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_24');

function request_shipping_quote_method_24() {
	if (!class_exists('WC_Paq_Light_Internacional')) {

		class WC_Paq_Light_Internacional extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_24');

function add_request_shipping_quote_24( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Light_Internacional::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Light_Internacional';
	}
