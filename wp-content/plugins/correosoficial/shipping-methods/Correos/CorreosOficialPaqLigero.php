...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_21');

function request_shipping_quote_method_21() {
	if (!class_exists('WC_Paq_Ligero')) {

		class WC_Paq_Ligero extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_21');

function add_request_shipping_quote_21( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Ligero::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Ligero';
	}
