...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_23');

function request_shipping_quote_method_23() {
	if (!class_exists('WC_Paq_Estandar_Internacional')) {

		class WC_Paq_Estandar_Internacional extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_23');

function add_request_shipping_quote_23( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Estandar_Internacional::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Estandar_Internacional';
	}
