...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_17');

function request_shipping_quote_method_17() {
	if (!class_exists('WC_Paq_Premium_City_Paq')) {

		class WC_Paq_Premium_City_Paq extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_17');

function add_request_shipping_quote_17( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Paq_Premium_City_Paq::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Paq_Premium_City_Paq';
	}
