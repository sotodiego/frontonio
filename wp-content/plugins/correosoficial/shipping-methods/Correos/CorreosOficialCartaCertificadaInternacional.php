...
add_action('woocommerce_shipping_init', 'request_shipping_quote_method_26');

function request_shipping_quote_method_26() {
	if (!class_exists('WC_Carta_Certificada_Internacional')) {

		class WC_Carta_Certificada_Internacional extends WC_Shipping_Method {
...
add_filter('woocommerce_shipping_methods', 'add_request_shipping_quote_26');

function add_request_shipping_quote_26( $methods ) {

	$shippingMethod = CorreosOficialCommonShippingMethods::loadShippingMethod(WC_Carta_Certificada_Internacional::getPaqId());

	if ($shippingMethod) {
		$methods['request_shipping_quote_' . $shippingMethod[0]['id']] = 'WC_Carta_Certificada_Internacional';
	}
