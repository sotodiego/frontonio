...
require_once(dirname(__FILE__).'/include/tcpdf_filters_cex.php');
...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, TCPDF_FILTERS_CEX::getAvailableFilters())) {
				try {
					$stream = TCPDF_FILTERS_CEX::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
