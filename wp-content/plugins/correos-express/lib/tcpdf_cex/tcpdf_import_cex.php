...
require_once(dirname(__FILE__).'/tcpdf_cex.php');
// include PDF parser class
require_once(dirname(__FILE__).'/tcpdf_parser_cex.php');
 ...
 */
class TCPDF_IMPORT_CEX extends TCPDF_CEX {
	...
	public function importPDF($filename) {
		...
		try {
			// parse PDF data
			$pdf = new TCPDF_PARSER_CEX($rawdata, $cfg);
		} catch (Exception $e) {
			die($e->getMessage());
		}
		// get the parsed data
