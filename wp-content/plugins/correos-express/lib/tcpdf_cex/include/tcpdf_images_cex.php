...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (empty($type)) {
			...
			if (isset($fileinfo['extension']) AND (!TCPDF_STATIC_CEX::empty_string($fileinfo['extension']))) {
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (TCPDF_STATIC_CEX::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = TCPDF_STATIC_CEX::_freadint($f);
		$h = TCPDF_STATIC_CEX::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = TCPDF_STATIC_CEX::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = TCPDF_STATIC_CEX::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = TCPDF_STATIC_CEX::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= TCPDF_STATIC_CEX::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = TCPDF_STATIC_CEX::rfread($f, ($n - $len - 2));
				// decompress profile
