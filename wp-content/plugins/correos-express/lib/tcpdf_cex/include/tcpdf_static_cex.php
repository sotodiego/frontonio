<?php
//============================================================+
// File name   : TCPDF_STATIC_CEX.php
// Version     : 1.1.3
// Begin       : 2002-08-03
...
class TCPDF_STATIC_CEX {
	...
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(TCPDF_STATIC_CEX::getRandomSeed()).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = TCPDF_COLORS_CEX::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = TCPDF_COLORS_CEX::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		foreach($patterns_array as $val) {
			if (!TCPDF_STATIC_CEX::empty_string($val)) {
				$val = trim($val);
				$val = str_replace('\'', '\\\'', $val);
