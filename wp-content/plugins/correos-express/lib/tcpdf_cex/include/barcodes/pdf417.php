...
class PDF417 {
		...
		switch($mode) {
			...
			case 902: { // Numeric Compaction mode latch
				while (($codelen = strlen($code)) > 0) {
					...
					$t = '1'.$code;
					do {
						$d = $this->bcmod_A($t, '900');
						$t = $this->bcdiv_A($t, '900');
						array_unshift($cw, $d);
					} while ($t != '0');
					$code = $rest;
				}
	...
	/**
 * bcmod_A - simula la función bcmod
 **/

public function bcmod_A( $x, $y )
{
    $numeros = 5;    //número (int) que toma en cada llamada
    $resto = '';
    do
    {
        $a = (int)$mod.substr( $x, 0, $numeros );
        $x = substr( $x, $numeros );
        $resto = $a % $y;
    }
    while ( strlen($x) );
...
public function bcdiv_A( $first, $second, $scale = 0 )
{
    $res = $first / $second;
